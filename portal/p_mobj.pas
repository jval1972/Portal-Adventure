...
//==============================================================================
//
// P_ClearMobjCustomParams
//
//==============================================================================
procedure P_ClearMobjCustomParams(const actor: Pmobj_t);
...
//
//==============================================================================
function PlayerInWalkFrame(const player: Pplayer_t): boolean;
begin
  if player.mo.state = @states[Ord(S_PLAY_RUN1)] then
  begin
    Result := True;
    Exit;
  end;
  if player.mo.state = @states[Ord(S_PLAY_RUN2)] then
  begin
    Result := True;
    Exit;
  end;
  if player.mo.state = @states[Ord(S_PLAY_RUN3)] then
  begin
    Result := True;
    Exit;
  end;
  if player.mo.state = @states[Ord(S_PLAY_RUN4)] then
  begin
    Result := True;
    Exit;
  end;
  if player.mo.state = @states[Ord(S_PLAY_RUN5)] then
  begin
    Result := True;
    Exit;
  end;
  if player.mo.state = @states[Ord(S_PLAY_RUN6)] then
  begin
    Result := True;
    Exit;
  end;
  if player.mo.state = @states[Ord(S_PLAY_RUN7)] then
  begin
    Result := True;
    Exit;
  end;
  if player.mo.state = @states[Ord(S_PLAY_RUN8)] then
  begin
    Result := True;
    Exit;
  end;
  if player.mo.state = @states[Ord(S_PLAY_RUNFAST1)] then
  begin
    Result := True;
    Exit;
  end;
  if player.mo.state = @states[Ord(S_PLAY_RUNFAST2)] then
  begin
    Result := True;
    Exit;
  end;
  if player.mo.state = @states[Ord(S_PLAY_RUNFAST3)] then
  begin
    Result := True;
    Exit;
  end;
  if player.mo.state = @states[Ord(S_PLAY_RUNFAST4)] then
  begin
    Result := True;
    Exit;
  end;
  if player.mo.state = @states[Ord(S_PLAY_RUNFAST5)] then
  begin
    Result := True;
    Exit;
  end;
  if player.mo.state = @states[Ord(S_PLAY_RUNFAST6)] then
  begin
    Result := True;
    Exit;
  end;
...
begin
  ...
  begin
    ...
    if (player <> nil) and PlayerInWalkFrame(player) then
...
begin
  ...
  p := mo.player;
  if (p <> nil) and (mo.z < mo.floorz) then
  begin
    p.viewheight := p.viewheight - (mo.floorz - mo.z);
    p.deltaviewheight :=
      _SHR((PVIEWHEIGHT - p.viewheight), 3);
  end;
  ...
  begin
    ...
    if mo.momz < 0 then
    begin
      if (p <> nil) and (mo.momz < -GRAVITY * 8) then
      begin
        // Squat down.
        // Decrease viewheight for a moment
        // after hitting the ground (hard),
        // and utter appropriate sound.
        p.deltaviewheight := _SHR(mo.momz, 3);
  ...
  begin
    ...
    if (p <> nil) and (p.lowgravtics > 0) and lowgrav then
    begin
      if mo.momz >= 0 then
        mo.momz := mo.momz - grav div 2;
    end
    else
    begin
      if mo.momz = 0 then
...
//  mobj.interactdata := 0; // JVAL: entities interaction
...
begin
  ...
  begin
    I_Warning('P_SpawnMapThing(): Unknown type %d at (%d, %d)',
  ...
  result.angle := mthing.angle * ANG1; // ANG45 * (mthing.angle div 45);
  if mthing.options and MTF_AMBUSH <> 0 then
...
begin
  ...
  begin
    i := 0;
    while i < p.numparams do
    begin
      if p.params[i].name = strupper(name) then
        break;
      inc(i);
    end;
    p.params[i].name := strupper(name);
    p.params[i].value := value;
    if i = p.numparams then
      Inc(p.numparams);
  end;
end;
...
procedure P_ClearMobjCustomParams(const actor: Pmobj_t);
begin
  // JVAL: Deallocation is lazy,
  //   but suprosingly we don't have memory leaks, guess why :) :)
  actor.customparams := nil;
end;
