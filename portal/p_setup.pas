...
//
//==============================================================================
procedure P_IdendifyGameParams;
...
begin
  ...
  begin
    if @think._function.acp1 = @P_MobjThinker then
      if Pmobj_t(think).info.doomednum = 21091 then
      begin
        mission1 := Pmobj_t(think);
        break;
      end;
    think := think.next;
  end;
...
begin
  ...
  printf(#13#10'-------------'#13#10);
  printf('Loading LEVEL %d'#13#10, [map]);
 ...
 { if lumpnum + Ord(ML_CODE) < W_NumLumps then
  ...
  begin
    ...
    printf(' Extended GL nodes v%d found!'#13#10, [glnodesver, glmapname]);
