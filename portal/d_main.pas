...
begin
  ...
  SUC_SetGameMode('The hunt for Dr. Freak');

  printf('M_InitArgv: Initializing command line parameters.'#13#10);
  M_InitArgv;
  ForceDirectories(M_SaveFileName('GAMESAV\'));
  ...
  printf(
           '                         ' +
           'Portal Adventure - The hunt for Dr. Freak v%d.%d' +
           '                           '#13#10,
  ...
  if M_CheckParmCDROM then
    basedefault := CD_WORKDIR + 'portal.ini'
  else
    basedefault := 'portal.ini';
...
// Try to guess minimum zone memory to allocate
  mb_min := 1 + V_ScreensSize(SCN_FG) div (1024 * 1024);
  ...
  SUC_Progress(23);
  lc1 := I_FileCheckSum('main.dat');
  SUC_Progress(28);
  lc2 := I_FileCheckSum('maps.dat');
  SUC_Progress(29);
  lc3 := I_FileCheckSum('base.dat');

  printf('CheckSums: main.dat=%d, maps.dat=%d, base.dat=%d'#13#10, [lc1, lc2, lc3]);
  ...
  printf('CM_Init: Initializing interactive comic data.'#13#10);   // comic!!
  CM_Init;

{  if (lc1 <> 985825563) or (lc2 <> 3287673519) or (lc3 <> 66313943) then
    I_Error('Correpted data detected, can not continue.');}
...
begin
  ...
  C_ShutDown;
  printf('CM_ShutDown: Shut down interactive comic data.'#13#10);
  CM_ShutDown;
