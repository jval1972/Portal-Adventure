...
begin
  ...
  fullscreenbubble := false;
  if thing.comicdata <> nil then
    if (Pdialogdef_t(thing.comicdata).flags and CMDLG_FULLSCREEN) <> 0 then
      fullscreenbubble := true;
  ...
  if not fullscreenbubble then
  begin
    // thing is behind view plane?
    checksides := (absviewpitch < 15) and (thing.state.dlights = nil) and (thing.state.models = nil);
  ...
  dbg_sprite := thing.sprite;
  if dbg_sprite >= numsprites then
  begin
    printf('R_ProjectSprite(): "%s" - fuck!.'#13#10, [thing.info.name]);
    if dbg_lump < 0 then
      dbg_lump := W_GetNumForName('TNT1A0');
    lump := dbg_lump;
...
begin
  ...
  if P_Distance(thing.x - viewx, thing.y - viewy) > 384 * FRACUNIT + thing.radius then
    if (thing.comicdata = nil) or ((Pdialogdef_t(thing.comicdata).flags and CMDLG_FULLSCREEN) = 0) then
      exit;
